...
import { convertReaperColorToCssColor, convertReaperColorToGrandmaAppearanceColor, createInheritedRegionBumpColor, createInheritedRegionLayerColor, } from "../src/lib/reaper2ma/colors.js";
...
function getTimecodeTrackCommands(commands, tempDataPoolName, sequenceIndex) {
...
`;
        ...
        assert.deepEqual(artifacts.regionLayerSequences.map((sequence) => `${sequence.regionId}:${sequence.layerName}`), ["R1:FX", "R1:Voix", "R2:FX"]);
...
`;
        ...
        assert.equal(artifacts.validationWarnings.some((warning) => warning.includes("[OFF_LAYER=Missing]") && warning.includes("does not exist")), true);
        assert.equal(artifacts.validationWarnings.some((warning) => warning.includes("[OFF_LAYER=FX]") && warning.includes("without a target region")), true);
    });
...
`;
        ...
        assert.deepEqual(artifacts.bumpSequences.map((sequence) => ({
            color: sequence.color,
            displayName: sequence.displayName,
            appearanceName: sequence.appearanceName,
            appearanceNumber: sequence.appearanceNumber,
...
describe("zip export bundle", () => {
    ...
    it("rejects empty archives and duplicate entry names", () => {
        ...
        assert.throws(() => createZipArchiveBytes([
