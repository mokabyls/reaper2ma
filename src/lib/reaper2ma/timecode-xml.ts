...
import type { BpmSequence, BumpSequence, ConversionSettings, ConvertedMarker, RegionSequence, RepeatedSequence, SequenceTrigger } from "./types.js";
...
type TimecodeEvent = {
    "@_Name": string;
    "@_Time": string;
    RealtimeCmd: Record<string, string>;
};
...
function createSequenceCueDestination(sequenceName: string, cueName: string): string {
    return `ShowData.DataPools.Default.Sequences.${sequenceName}.${cueName}`;
}

function createRegionActionDestination(regionId: string, kind: "ON" | "OFF"): string {
    return kind === "ON"
        ? `ShowData.DataPools.Default.Sequences.${regionId}.Cue 1`
        : `ShowData.DataPools.Default.Sequences.${regionId}`;
}

function createRegionActionExecToken(kind: "ON" | "OFF"): string {
    return kind === "ON" ? "Goto|Go+" : "Off";
}

function createEventsForMarker(
    marker: {
        timestamp: string;
        execToken: string;
        regionActions?: SequenceTrigger["regionActions"];
    },
    target: string,
    destination: string,
    isFirstEvent: boolean,
    ...
    for (const action of regionActions) {
        events.push({
            ...
            RealtimeCmd: createRealtimeExecutionCommand(
                createRegionActionExecToken(action.kind),
                `ShowData.DataPools.Default.Sequences.${action.regionId}`,
                createRegionActionDestination(action.regionId, action.kind),
                false,
            ),
        });
    }
...
function createEventsForUniqueCues(sequenceNumber: number, uniqueCues: ConvertedMarker[]) {
    const cuePlan = createUniqueCuePlan(uniqueCues);

    return cuePlan.flatMap((item, index) =>
        createEventsForMarker(
            {
            ...
            createSequenceCueDestination(`Sequence ${sequenceNumber}`, item.cueName),
            index === 0,
            item.execToken,
        ),
    );
...
function createEventsForRepeatedSequence(sequenceName: string, events: RepeatedSequence["events"]) {
    return events.flatMap((event, index) =>
        createEventsForMarker(
            ...
            createSequenceCueDestination(sequenceName, event.cueName),
            index === 0,
            event.execToken,
        ),
    );
...
function createEventsForRegionSequence(sequenceName: string, events: RegionSequence["events"]) {
    return events.flatMap((event, index) =>
        createEventsForMarker(
            {
                timestamp: event.timestamp,
...
function createEventsForBumpSequence(sequenceName: string, events: BumpSequence["events"]) {
    return events.flatMap((event, index) =>
        createEventsForMarker(
            {
                timestamp: event.timestamp,
...
function createColorSequenceTrack(sequenceName: string, events: RepeatedSequence["events"]) {
    return {
        "@_Guid": generateGuid(),
...
function createRegionSequenceTrack(sequenceName: string, events: RegionSequence["events"]) {
    return {
        "@_Guid": generateGuid(),
        "@_Target": `ShowData.DataPools.Default.Sequences.${sequenceName}`,
        "@_Play": "",
...
function createBumpSequenceTrack(sequenceName: string, events: BumpSequence["events"]) {
    return {
        "@_Guid": generateGuid(),
        "@_Target": `ShowData.DataPools.Default.Sequences.${sequenceName}`,
        "@_Play": "",
...
export function generateTimecodeXML(
    ...
    const trackGroups: Array<Record<string, any>> = [
        {
            "@_Play": "",
            "@_Rec": "",
            MarkerTrack: {
